...
`tests/test_core.gd` が領域別の検査でこれを守る。特に重要なのは次の 2 つ。
...
| 曲 | 調性 | 狙い |
|---|---|---|
| `title` | イ短調・疎な合唱とハープ | 世界の外から未知の一回を眺める |
| `stronghold` | 長調・堂々とした歩調 | 古典的な和声を素直に辿る、王宮的な品 |
| `world` | 明暗を行き来する旅の拍 | 一度きりの世界を進み続ける |
| `town` | ハ長調・小編成 | 閉じる世界の中にある仮の故郷 |
| `cave` | ニ短調・疎な残響 | 世界の傷口へ降りる空洞感 |
| `story` | イ短調・心拍に近い低音 | 一人と一つの約束へ焦点を寄せる |
| `battle` | 短調・速い 24 小節 | 通常戦の判断を急がせる |
| `boss` | 短調・機械的反復 | 帝国機械の圧力に人の旋律が抗う |
| `chronicle` | 短調から安定終止へ | 成否のどちらも記録として砦へ残す |

`tools/verify_audio.py` が無音・クリップ・直流成分・音程を機械的に検算する
（耳で確認できない環境でも壊れを検出できる）。
各曲と場面 SE の意図は `docs/audio_design.md` にまとめている。
...
テンキーでは `2` `4` `6` `8` で移動、`5` またはテンキー Enter で決定、
`0` またはテンキー小数点でキャンセルできる。キー設定を変えてもテンキー操作は残る。
設定の「オートの道具」で、オート中の蘇生・瀕死回復・状態回復を許可できる。
既定は「使わない」で、攻撃道具・加速薬・MP回復は許可時も自動消費しない。
洞の入口には上り階段があり、2階以降では直前の階、1階では元のフィールドへ戻る。
セーブを最初からやり直すときは、タイトルで `X` →「セーブを けす」。
戦績・資源・熟練・中断を消すが、音量とキー割り当ては残る。
探索中に `Z` でメニュー（どうぐ / つよさ / そうび）が開く。

```bash
python tools/test_core.py                             # コア検算（領域別・並列）
python tools/test_core.py --suite combat --verbose   # 変更した領域だけ詳しく
godot --headless --script res://tests/test_core.gd   # 従来どおり全領域を1プロセスで実行
godot --headless --script res://tests/test_world_events.gd # 任意イベント品質Gate
python tools/verify_audio.py                          # 音の検算
godot --path . -- --shot=battle                       # 画面を 1 枚撮って終了
godot --path . -- --inspect=town_talk                 # 指定画面へ直行し、そのまま操作
```

`--inspect=` は `--shot=` と同じ実ゲーム状態を開くが、自動終了しない。
画面を見ながら通常のキーで操作でき、デバッグ中は実セーブを読み書きしない。
町の施設は `town_facility` で初回効果、`town_facility_repeat` で再利用時を直接確認できる。
町の箱は `town_chest` で開ける直前、`town_chest_opened` で報酬表示へ直接入れる。
戦闘の開戦文は `battle_opening` で、敵が先に動く場合の通知を止めて確認できる。
`--play=8 --play-start=battle` なら、遭遇演出から初手までを実入力で短く回帰できる。
`--play=30 --input=keypad` なら、操作名を直接送らず物理テンキー入力で同じ経路を検査する。
個別画面はこれで確認し、`--play=` 単独はタイトルからの通し回帰に使う。
...
動くもの: 初回プロローグ → 拠点 → 出撃 → 世界探索 → 遭遇 → CTB 戦闘 →
町・洞 → 城の主 → 生還 or 全滅 → 戦記 → 拠点。
**始まりと終わりのある輪**として閉じている。

- **終わりがある** — 地下 10 階には下り階段が無く、主の間の扉だけがある。
  主を倒すと「生還」でランが終わる（`end_run(true)` へ至る唯一の経路）
- **ゴールドはラン内資源** — 道中の出店で消耗品・装備に換える。持ち帰れない。
  全回復と毒の治療は町の宿だけが担い、町へ戻る手間が代金になる。
  買い物を拠点ではなく道中に置いたのは、「今使うか、取っておくか」を
  潜っている最中に判断させたいから。拠点に置くと計画の問題になってしまう
- **道具も手番を消費する** — 回復に 1 手番を割く、という CTB 上の判断が成立する
- **なくならない戦具もある** — 宝箱で見つけるか、強化後に1つ選んで持ち込む。
  弱い属性攻撃は何度でも、回復は1戦1回にして、消耗戦を消さずに手を増やす。
  一部の武器・防具も戦闘中の「どうぐ」から使える
...
| | 得かた | 使いみち | ランをまたぐか |
|---|---|---|---|
| **ゴールド** | 戦闘・宝箱・ぬすむ | 道中の出店（消耗品・装備） | 失う |
...
アップグレードは能力値には触れない（旅支度・継承印の枠・商いの伝手・戦具の包み）。
レベルを失うことがランの代償なので、そこを恒久強化で埋めると代償そのものが消える。
手に入るのは、出撃時と道中で選べる戦い方。
...
`--runs` は歩き方1方針あたりの数で、100なら4方針合計400ランになる。
開発中は固定シード100回/方針で広い許容帯から外れる回帰を検知し、数値調整の確定時と
リリース前だけ500回/方針へ増やす。戦闘の長さだけなら40回/方針で十分な戦闘数になる。
現状は次の帯に合わせてある。
