...
動くもの: 探索 → 遭遇 → CTB 戦闘 → 勝敗 → 熟練度の持ち帰り → 次のラン、の輪が閉じている。

次に効く順:

1. **拠点画面** — 現状は全滅すると即座に次のランが始まる。転職・編成・熟練度確認の場が要る
2. **敵グラフィックの拡大** — ゲルは 48x40 に作り直したが、コウモリはまだ 32x32 で画面から浮く
3. **戦記のローカル AI 化** — `chronicle.gd` を Ollama に繋ぐ。接続点はすでに 1 か所に絞ってある
4. **アビリティの効果拡張** — かばう・状態異常など、CTB の行動順を動かす技を増やす
