...
気づいたことは行を足して、着手するときに上から取る。終わったら行を消し、
理由だけ「済んだこと」に残す（同じ判断を二度しないため）。

## 触ってよいもの / いけないもの

- `assets/` は**生成物**。直接編集しない。原本は `tools/gen_assets.py` と
  `tools/gen_audio.py` で、絵や音を変えるときは必ず生成器を直す。
- `docs/chara_image/` はユーザーが置いたキャラクター設定画。**勝手に描き換えない。**
  ただし `candidate_hero_<職業>.png`（72x128 の歩行シート）は生成器が取り込む素材で、
  `tools/gen_assets.py` がここを見て `assets/sprites/hero_*.png` を作る。
  取り込み時に SFC の制約（72x128 / 15 色 / アルファ 2 値 / BGR555）を検算し、
  外れていれば生成が止まる。素材が無ければ ASCII マップから起こす方に落ちる。
