...
SFC風の画面でも、ファミコン期の文字数制限をまねて平仮名へ開かない。
このゲームは漢字を読める14px以上で表示しているため、表記は現代のゲームUIに合わせる。

- 操作、分類、能力、状態、職業、品名は、常用される漢字で書く。
  `どうぐ / そうび / じゅくれんど / こうげき` ではなく、
  `道具 / 装備 / 熟練度 / 攻撃` とする。
- 短い選択肢は、先頭の漢字で見分けられる表記を優先する。
  平仮名だけの行を横に並べない。
- 助詞、助動詞、補助動詞は平仮名にする。文章全体を漢字へ詰め込まない。
- 常用表記でも読み違えやすい語、一般に平仮名で書く語は開いてよい。
  現在の許可例は `かばう / しもべ / されこうべ / じっくり`。
- 固有名詞を平仮名にする場合は、幼い人物の命名など作中の理由をデータ側へ記す。
  「レトロだから」は理由にしない。
- 一般的な語を平仮名へ戻す変更は `check_writing.py` で不合格にする。

この基準は文字の難しさだけでなく、一覧での検索速度を守るためにある。
`たたかう / じゅもん / とくぎ / どうぐ` は形が似て読み始めが遅いが、
`戦う / 魔法 / 特技 / 道具` なら先頭一字で区別できる。
...
- 戦記は勝敗、危険度、獲得ゴールドが実データと一致するときだけ採用する。
